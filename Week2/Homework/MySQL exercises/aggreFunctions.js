...
async function seedDatabase() {
    ...
    SELECT R.paper_id,paper_title,count(A.author_id) FROM research_papers R 
  ...
  try {
     ...
     execQuery( sumOf_female_researcher,(err ,result)=>{
      if(err) throw err;
      console.log('the Sum of the research papers published by all female authors is:',result);

    });
    
     execQuery( averageOf_H_index_perUnive,(err ,result)=>{
      if(err) throw err;
      console.log('Average of the h-index of all authors per university is:',result);

    });
     execQuery( sumOf_researchPapers_perUnive,(err ,result)=>{
      if(err) throw err;
      console.log('Sum of the research papers of the authors per university is:',result);

     });
     
     execQuery( minAndMax_Of_H_index_perUnive,(err ,result)=>{
      if(err) throw err;
      console.log('Minimum and maximum of the h-index of all authors per university is:',result);
